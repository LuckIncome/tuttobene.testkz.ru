                            ...
                            <div class="row">
                                <div class="col-lg-6 text-lg-start text-center">
                                    ...
                                    <a download="Презентация Франшизы Tutto Bene"
                                       href="{{Voyager::image(json_decode(setting('site.presentation'))[0]->download_link)}}" class="down_button">Скачать
                                        презентацию</a>
                                </div>
                ...
                <div class="accordion" id="accordionExample">
                    @foreach($textBlocks as $textBlock)
                        <div class="accordion-item">
                            <h2 class="accordion-header" id="heading{{\Str::slug($textBlock->title)}}">
                                <button class="accordion-button @if(!$loop->first) collapsed @endif" type="button"
                                        data-bs-toggle="collapse"
                                        data-bs-target="#collapse{{\Str::slug($textBlock->title)}}"
                                        aria-expanded="{{$loop->first ? 'true' : 'false'}}"
                                        aria-controls="collapse{{\Str::slug($textBlock->title)}}">
                                    {{$textBlock->title}}
                                </button>
                            </h2>
                            <div id="collapse{{\Str::slug($textBlock->title)}}"
                                 class="accordion-collapse collapse @if($loop->first) show @endif"
                                 aria-labelledby="heading{{\Str::slug($textBlock->title)}}"
